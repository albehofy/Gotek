...
export class ControllerDashboardComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    const token = localStorage.getItem('gotek_client_token');
    const userStr = localStorage.getItem('gotek_user');
    let user: any = null;
    try {
      user = userStr ? JSON.parse(userStr) : null;
    } catch (e) {}

    if (!token || !user || (user.role !== 'admin' && user.role !== 'Administrator' && user.role !== 'dashboard_controller')) {
      // Clear token & user info and redirect to login
      localStorage.removeItem('gotek_client_token');
