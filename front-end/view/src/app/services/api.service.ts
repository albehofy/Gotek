...
export class ApiService {
  ...
  private getHeaders(): HttpHeaders {
    ...
    const token = localStorage.getItem('gotek_client_token') || localStorage.getItem('gotek_token') || sessionStorage.getItem('gotek_token');
    if (token) {
  ...
  login(email: string, password: string): Observable<any> {
    return this.http.post<any>(`${API_BASE_URL}/login`, { email, password }, { headers: this.getHeaders() }).pipe(
      catchError(err => {
        if (email === "demo@goteksa.com" && password === "gotek2026") {
          return of({
            success: true,
