...
@Component({
        ...
        <div class="detail-drawer" [class.is-wide]="isWideDrawer" (click)="$event.stopPropagation()">
          ...
          <div class="drawer-hd">
              ...
              <div class="drawer-hd-text">
                ...
                <div class="drawer-meta">
                  <span class="priority-badge" [class]="'pb-' + (selectedTask.priority || 'medium')">
                    <i class="fa-solid" [ngClass]="getPriorityIcon(selectedTask.priority)"></i> {{ getPriorityLabel(selectedTask.priority) }}
                  </span>
            ...
            <div class="drawer-hd-actions">
              ...
              <!-- Delete Task Button -->
              <button class="icon-action-btn btn-del-task" *ngIf="isAdminOrManager()" (click)="confirmDeleteMainTask(selectedTask)" title="حذف المهمة نهائياً">
          ...
          <div class="drawer-body">
            ...
            <div class="drawer-tab-content" *ngIf="activeDrawerTab === 'details'">
              ...
              <div class="drawer-card priority-switcher-card" *ngIf="!isClient()">
                <div class="dc-head"><i class="fa-solid fa-bolt" style="color:var(--amber);"></i> مستوى الأولوية (تعديل الأولوية)</div>
                <div class="priority-steps-grid">
                  <button
                    type="button"
                    *ngFor="let p of priorityList"
                    class="priority-step-btn"
                    [class.active]="(selectedTask.priority || 'medium') === p.id"
                    [class]="'p-step-' + p.id"
                    (click)="updateTaskPriority(selectedTask, p.id)"
                  >
                    <i class="fa-solid" [ngClass]="getPriorityIcon(p.id)"></i>
                    <span class="p-step-name">{{ p.label }}</span>
                    <i class="fa-solid fa-check ps-check" *ngIf="(selectedTask.priority || 'medium') === p.id"></i>
                  </button>
                </div>
              </div>
                ...
                <div class="team-assign-container" style="display:flex; flex-direction:column; gap:12px; margin-top:6px;">
                  ...
                  <div class="assign-user-picker" *ngIf="!isClient()">
                    <app-prime-picker-select
                      ...
                      optionLabel="name"
                      optionValue="id"
                      optionSubLabel="email"
                      placeholder="+ انقر لاختيار وإسناد موظف للمهمة..."
                      (onChange)="onUserPickerSelect($event)"
                    ></app-prime-picker-select>
                  </div>
  ...
  styles: [`
    ...
    .icon-action-btn.btn-del-task {
      background: rgba(244, 63, 94, 0.1);
      border-color: rgba(244, 63, 94, 0.25);
      color: #fb7185;
    }
    .icon-action-btn.btn-del-task:hover {
      background: #f43f5e;
      color: #ffffff;
      border-color: #f43f5e;
    }
    ...
    ::ng-deep body > .prime-luxury-multiselect-panel,
    ...
    ::ng-deep .prime-luxury-multiselect-panel .p-multiselect-filter-container,
    ::ng-deep .p-multiselect-panel .p-multiselect-filter-container {
      position: relative !important;
      flex: 1 !important;
      width: 100% !important;
      display: flex !important;
    ...
    ::ng-deep .prime-luxury-multiselect-panel .p-multiselect-filter-input,
    ::ng-deep .prime-luxury-multiselect-panel .p-inputtext,
    ::ng-deep .p-multiselect-panel .p-multiselect-filter,
    ::ng-deep .p-multiselect-panel .p-inputtext {
      width: 100% !important;
    ...
    ::ng-deep .prime-luxury-multiselect-panel .p-multiselect-filter-icon,
    ::ng-deep .prime-luxury-multiselect-panel .p-inputicon,
    ::ng-deep .p-multiselect-panel .p-multiselect-filter-icon {
      position: absolute !important;
      right: 12px !important;
      left: auto !important;
      top: 50% !important;
    ...
    ::ng-deep .prime-luxury-multiselect-panel .p-multiselect-item-group,
    ::ng-deep .p-multiselect-panel .p-multiselect-item-group {
      background: rgba(99, 102, 241, 0.08) !important;
      color: var(--violet-light, #818cf8) !important;
    ...
    ::ng-deep .prime-luxury-multiselect-panel .p-checkbox .p-checkbox-box,
    ::ng-deep .p-multiselect-panel .p-checkbox .p-checkbox-box {
...
export class TasksBoardComponent implements OnInit {
  ...
  priorityList = [
    ...
    { id: 'high', label: 'عالية' },
    { id: 'urgent', label: 'عاجلة جداً' }
  ];

  statusList = [
    { id: 'new', label: 'جديد' },
    { id: 'in_progress', label: 'قيد التنفيذ' },
    { id: 'content_creator', label: 'صناعة المحتوى' },
    { id: 'in_review', label: 'قيد المراجعة' },
    { id: 'client_feedback', label: 'ملاحظات العميل' },
    { id: 'done', label: 'مكتمل' }
  ];
  ...
  onSubtaskUserSelect(eventOrId: any): void {
    if (!eventOrId) return;
    const uid = typeof eventOrId === 'object' && eventOrId.value !== undefined ? Number(eventOrId.value) : Number(eventOrId);
  ...
  getTasksForColumn(colKey: string) {
    let list = (this.tasks || []).filter(t => {
      ...
      if (colKey === 'in_progress') {
        return t.status === 'in_progress' || t.status === 'progress' || t.status === 'active';
      }
      if (colKey === 'content_creator') {
        return t.status === 'content_creator';
      }
      if (colKey === 'new') {
        return t.status === 'new' || t.status === 'todo' || t.status === 'pending' || !t.status;
      }
  ...
  approveTaskByClient(task?: any, event?: Event): void {
    ...
    const target = task || this.selectedTask;
    if (!target) return;
    const oldStatus = target.status;
    target.status = 'approved';
    this.apiService.approveClientTask(target.id).subscribe({
      next: () => {
        this.apiService.addTaskNote(target.id, 'تمت موافقة واعتماد المهمة بنجاح من قِبل العميل VIP').subscribe(() => {
          this.loadTaskActivities(target.id);
        });
      },
      error: () => {
        this.persistStatusUpdate(target, 'approved', () => target.status = oldStatus);
      }
    });
  }
  ...
  confirmDeleteMainTask(task: any): void {
    if (!task) return;
    this.confirmService.confirm({
      title: 'تأكيد حذف المهمة',
      message: `هل أنت متأكد من رغبتك في حذف المهمة "${task.title}" ومرفقاتها وبياناتها نهائياً؟`,
      confirmText: 'نعم، حذف المهمة',
      cancelText: 'إلغاء وتراجع',
      type: 'danger',
      ...
      accept: () => {
        this.apiService.deleteTask(task.id).subscribe({
          next: () => {
            this.toastService.success(`تم حذف المهمة "${task.title}" بنجاح`, 'تم الحذف');
            this.closeTaskDrawer();
            this.loadData();
          },
          error: (err) => {
            this.toastService.error(err.error?.message || 'تعذر حذف المهمة', 'خطأ بالحذف');
          }
        });
      }
    });
  ...
  onUserPickerSelect(eventOrId: any): void {
    if (!eventOrId) return;
    const uid = typeof eventOrId === 'object' && eventOrId.value !== undefined ? Number(eventOrId.value) : Number(eventOrId);
    if (uid) {
      this.assignUserById(uid);
    }
  }

  unassignedUsers(): any[] {
    if (!this.selectedTask) return this.allUsers || [];
    const assignedIds = (this.selectedTask.users || []).map((u: any) => u.id);
    ...
    if (!exists) {
      ...
      this.apiService.assignTaskMembers(this.selectedTask.id, userIds).subscribe({
        next: () => {
          ...
          this.toastService.success(`تم إسناد المهمة للموظف "${userObj?.name || ''}" بنجاح`, 'تم الإسناد');
          this.loadTaskActivities(this.selectedTask.id);
        },
        error: (err) => {
          this.toastService.error(err.error?.message || 'تعذر إسناد المهمة');
        }
      });
    }
  }
  ...
  updateTaskPriority(task: any, newPriority: string): void {
    if (!task || task.priority === newPriority) return;
    const oldPriority = task.priority;
    task.priority = newPriority;

    this.apiService.updateTaskPriority(task.id, newPriority).subscribe({
      next: () => {
        this.toastService.success(`تم تعديل أولوية المهمة إلى "${this.getPriorityLabel(newPriority)}" بنجاح`, 'تم تحديث الأولوية');
        this.loadTaskActivities(task.id);
      },
      error: (err) => {
        task.priority = oldPriority;
        this.toastService.error(err.error?.message || 'تعذر تعديل أولوية المهمة');
      }
    });
  }
  ...
  getPriorityIcon(priority: string): string {
    switch (priority) {
      case 'urgent': return 'fa-fire';
      case 'high': return 'fa-angles-up';
  ...
  getPriorityLabel(priority: string): string {
    const p = (this.priorityList || []).find((x: any) => x.id === priority);
    return p ? p.label : (priority || 'متوسطة');
  }
